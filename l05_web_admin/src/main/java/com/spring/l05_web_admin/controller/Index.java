...
public class Index {
  ...
  public String loginPage(@ModelAttribute("message") String message, Model model, HttpSession session) {
  ...
  public String login(RedirectAttributes redirectAttr, User user, HttpSession session) {
  ...
  public String logout(RedirectAttributes redirectAttr, HttpSession session, Model model) {
    if (session.getAttribute("loginUser") == null) {
      return "redirect:/login";
    } else {
